...
export function createPermutationOp(obj) {
    ...
    const rfb_user1_id_type = (user1_id_type === 'CPF') ? 1 
    : (user1_id_type === 'CNPJ') ? 2 
    : (user1_id_type === 'NIF_PF') ? 3 
    : (user1_id_type === 'NIF_PJ') ? 4
    : (user1_id_type === 'PASSPORT') ? 5
    : (user1_id_type === 'COUNTRY_NO_ID') ? 6
    : (user1_id_type === 'USER_NO_ID') ? 7
    : '';

    const rfb_user1_cpf = (user1_id_type === 'CPF' || user1_id_type === 'CNPJ') ? user1_document.match(/\d+/g).join('') : '';
    const rfb_user1_nif = (user1_id_type === 'NIF_PF' || user1_id_type === 'NIF_PJ' || user1_id_type === 'PASSPORT') ? user1_document.match(/\d+/g).join('') : '';

    const rfb_user2_id_type = (user1_id_type === 'CPF') ? 1 
    : (user2_id_type === 'CNPJ') ? 2 
    : (user2_id_type === 'NIF_PF') ? 3 
    : (user2_id_type === 'NIF_PJ') ? 4
    : (user2_id_type === 'PASSPORT') ? 5
    : (user2_id_type === 'COUNTRY_NO_ID') ? 6
    : (user2_id_type === 'USER_NO_ID') ? 7
    : '';

    const rfb_user2_cpf = (user2_id_type === 'CPF' || user2_id_type === 'CNPJ') ? user2_document.match(/\d+/g).join('') : '';
    const rfb_user2_nif = (user2_id_type === 'NIF_PF' || user2_id_type === 'NIF_PJ' || user2_id_type === 'PASSPORT') ? user2_document.match(/\d+/g).join('') : '';
    
    return `${line_type}|${date}|${id}|${operation_code}|${rfb_brl_fees}|${user1_coin_symbol}|${rfb_user1_coin_quantity}|${rfb_user1_id_type}|${user1_country}|${rfb_user1_cpf}|${rfb_user1_nif}|${user1_fullname}|${user1_address}${user2_coin_symbol}|${rfb_user2_coin_quantity}|${rfb_user2_id_type}|${user2_country}|${rfb_user2_cpf}|${rfb_user2_nif}|${user2_fullname}|${user2_address}\r\n`;
}